import type { CellData, CursorState } from "./types.js";
    ...
    case "scroll":
      return [
        ...
        message.direction === "up"
          ? WireScrollDirection.Up
          : WireScrollDirection.Down,
      ];
    case "listTerminals":
...
function decodeSnapshot(message: unknown[]): SnapshotMessage {
  return {
    ...
    scrollbackLineLens: arrayAt(message, 8).map((value) =>
      numberValue(value),
    ),
...
function decodeCells(value: unknown): CellData[] {
  return arrayValue(value).map((cell) => {
    ...
    if (packed.length > 4 && packed[4] !== null) {
      out.fgRgb = numberAt(packed, 4);
    }
    if (packed.length > 5 && packed[5] !== null) {
      out.bgRgb = numberAt(packed, 5);
    }
    return out;
  });
}
